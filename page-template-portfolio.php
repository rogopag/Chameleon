...
<div id="content" class="clearfix<?php if($fullwidth) echo(' fullwidth');?>">
	...
	<?php if ( have_posts() ) : while ( have_posts() ) : the_post(); ?>
		<div class="entry post clearfix">							
							...
							<div class="et_pt_portfolio_item">
								...
								<div class="et_pt_portfolio_entry<?php if ( $height == $et_portrait_height ) echo ' et_portrait_layout'; ?>">
									<div class="et_pt_portfolio_image<?php if ($et_videolink <> '') echo ' et_video'; ?>">
										...
										<a class="et_portfolio_more_icon" href="<?php the_permalink(); ?>"><?php esc_html_e('Continua','Chameleon'); ?></a>
									</div> <!-- end .et_pt_portfolio_image -->
								</div> <!-- end .et_pt_portfolio_entry -->
								<?php if ($et_ptemplate_showdesc) { ?>
								<p> <?php #truncate_post(90); ?> <?php global $more; $more = 0; ?> <?php the_content(); ?> </p>
								<?php } ?>
			...
			<?php	
			$newLink = get_post_meta($post->ID, 'newLink', true);
			$newText = get_post_meta($post->ID, 'newText', true); ?>
			<h4><p style="text-align: center;"><a href=" <?php echo esc_url ($newLink); ?> "> <?php echo $newText; ?> 				</a></p></h4>

			<?php	
			$loopLink= get_post_meta($post->ID, 'loopLink', true);
			$loopText= get_post_meta($post->ID, 'loopText', true); ?>
			<h5><p style="text-align: center;"><a href=" <?php echo esc_url ($loopLink); ?> "> <?php echo $loopText; ?> </a></p></h5>
